...
def pdf_to_images_zip(pdf_path, zip_path, temp_output_path, img_format=ImageFormats.JPG):
    try:
...
def images_zip_to_pdf(zip_path, pdf_path, temp_output_path, img_format=ImageFormats.JPG):
    unzip_to_folder(zip_path, temp_output_path)
    with open(pdf_path, "wb") as new_pdf:
        imgs = []
        for fname in sorted(os.listdir(temp_output_path)):
            if not fname.endswith("."+img_format):
                continue

            path = os.path.join(temp_output_path, fname)
...
def init_args():
    ...
    parser.add_argument('-i', '--input', required=True, help="path of in pdf to convert")
    parser.add_argument('-o', '--output', required=True, help="path of converted pdf")
...
def main():
    ...
    pdf_to_images_zip(args.input, "converted.zip", "tmp")
    os.makedirs("tmp2")
    images_zip_to_pdf("converted.zip", args.output, "tmp2")
